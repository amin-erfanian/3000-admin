<template>
  <div class="date-picker">
    <div class="date-picker__header">
      ...
      <div v-if="!showYearMonthPicker" class="date-picker__month-selection">
      ...
      <div v-if="!showYearMonthPicker" class="month-days-selector">
        ...
        <div class="days">
          ...
          >
            ...
            <div
              v-if="isDateInDues(day)"
              class="bullet"
              :class="{ 'over-due': datePassed(day) }"
            ></div>
          </div>
        </div>
      </div>
...
<script setup>
  ...
  const isDateInDues = (day) => {
    const date = `${state.year}/${state.month + 1}/${day}`;
    const isIn = props.dueDates.includes(date);
    return isIn;
  };

  const datePassed = (day) => {
    const { jy, jm, jd } = jToday;

    return (
      state.year < jy ||
      (state.year === jy && state.month + 1 < jm) ||
      (state.year === jy && state.month + 1 === jm && day < jd)
    );
  };
</script>
