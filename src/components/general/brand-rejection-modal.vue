<template>
  <BaseModal v-model="showModal" title="رد برند" width="760px">
    <div class="brand-rejection-modal">
      <div class="brand-rejection-modal__title"
        >ویژگی‌های دارای ایراد را انتخاب کنید</div
      >

      <div class="brand-rejection-modal__selection">
        <BaseSelect
          v-model="selectedProperties"
          :options="brandPropertyOptions"
          :multiple="true"
          option-label="name"
          option-value="value"
          label="ویژگی‌ها"
      ...
      <BaseTextArea
        v-model="hint"
        name="rejection-hint"
        label="توضیحات یا راهنمای رد برند"
      />

      <div class="brand-rejection-modal__actions">
...
<script setup>
  ...
  const brandPropertyOptions = computed(() => {
    if (props.propertyOptions.length) return props.propertyOptions;

    if (!props.brand || typeof props.brand !== 'object') return [];

    return Object.keys(props.brand).map((key) => ({
      name: key,
      value: key,
    }));
  });
